...
#define DEBUG(x, fmt, ...) do { \
                            if (x > 0x1000 || (x >= 0x2290000000 && x < 0x3a8fffffff+1)) { \
...
// DEBUG(addr, "%s %s\n", __func__, memory_region_name(mr));
    ...
    if (buffer != bounce.buffer) {
        ...
        assert(bptr != &bounce);
        // if(bptr && bptr != &bounce) {
        //     printf("bptr: %p, bounce: %p, buffer: %p\n", bptr, &bounce, buffer);
        //     for (int idx = 0; idx < bbm.num_buffers; idx++) {
        //         printf("\tbbm.buffers[%d]: %p\n", idx, bbm.buffers[idx]);
        //     }
        // }   

        // printf("bptr(%p): %p, bounce(%p): %p, buffer: %p\n", bptr, bptr->buffer, &bounce, bounce.buffer, buffer);
        // for (int idx = 0; idx < bbm.num_buffers; idx++) {
        //     printf("\tbbm.buffers[%d](%p): %p, %d\n", idx, bbm.buffers[idx], bbm.buffers[idx]->buffer, bbm.buffers[idx]->in_use);
        // }
