                                        ...
                                        MemTxAttrs attrs)
{
    ...
    // if (mr->ops == &cfmws_ops)
        // print_now("before region dispatch read1");
    r = memory_region_dispatch_read1(mr, addr, pval, size, attrs);
    // if (mr->ops == &cfmws_ops)
        // print_now("after region dispatch read1");
...
void memory_region_init_ram_ptr_nomigrate(MemoryRegion *mr,
                                        Object *owner,
                                        const char *name,
                                        uint64_t size,
                                        void *ptr)
...
void memory_region_init_ram_ptr_dual_mode(MemoryRegion *mr,
    Object *owner,
    const char *name,
    uint64_t size,
    void *ptr)
...
void address_space_init(AddressSpace *as, MemoryRegion *root, const char *name)
{
    ...
    address_space_update_ioeventfds(as);
    // printf("%s as_name:%s, mr_name:%s base_addr:0x%lx, size:0x%lx, iommu(%d) \n",__func__, name, root->name, root->addr, (uint64_t)root->size, root->is_iommu);
}
...
void memory_region_init_dual(MemoryRegion *mr,
                            Object *owner,
                            const MemoryRegionOps *ops,
                            void *opaque,
                            const char *name,
