...
void breakpoint_empty(void) ;
void breakpoint_empty(void) {
    // printf("breakpoint_empty\n");
    return;
}

static inline bool __addr_ok(hwaddr addr) {
    // return (addr >= 0x100000000 && addr < 0x480000000) || (addr >= 0x2290000000 && addr < 0x3A90000000);
    return (addr >= 0x2290000000 && addr < 0x3A90000000);
}
#define DEBUG(x, fmt, ...) do { \
                            if (__addr_ok(x)) { \
                                        ...
                                        hwaddr l, MemoryRegion *mr)
{
    ...
    // DEBUG(addr, "%s addr:0x%lx, addr1:0x%lx, len:0x%lx, l:0x%lx, mr:%s, ptr(0x%lx) val: 0x%lx\n",__func__, addr, addr1, len, l, mr->name, (uint64_t)ptr, *(uint64_t*)ptr)
    for (;;) {
                                ...
                                MemoryRegion *mr)
{
    ...
    for (;;) {
        if (!flatview_access_allowed(mr, attrs, addr1, l)) {
            ...
            //     offset = addr - femu->base_gpa;
            //     printf("%s addr: 0x%lx, offset: 0x%lx (base: 0x%lx), l:%ld\n", __func__, addr, offset, femu->base_gpa, l);
            //     femu->cxl_mem_ops.read((void *)femu, offset & (~(TARGET_PAGE_SIE-1)), &dummy, sizeof(dummy), MEMTXATTRS_UNSPECIFIED);

            //     ram_ptr = qemu_ram_ptr_length(mr->ram_block, addr, &l, false);
            ...
            // else {
                ...
                // if (mr->ops == &cfmws_ops)
                    // print_now("before dispatch read");
                result |= memory_region_dispatch_read(mr, addr1, &val,
                                                    size_memop(l), attrs);
                // if (mr->ops == &cfmws_ops)
                    // print_now("after dispatch read");
        ...
        // DEBUG(addr, "%s_end addr:0x%lx, addr1:0x%lx, len:0x%lx, l:0x%lx, mr:%s, ptr(0x%lx) val: 0x%lx\n",__func__, addr, addr1, len, l, mr->name, (uint64_t)ptr, *(uint64_t*)ptr)
    return result;
}
                                ...
                                MemTxAttrs attrs, void *buf, hwaddr len)
{
    ...
    // if (mr->ops == &cfmws_ops)
        // print_now("before flatview read cont");
    res = flatview_read_continue(fv, addr, attrs, buf, len,
                                addr1, l, mr);
    // if (mr->ops == &cfmws_ops)
        // print_now("after flatview read cont");
...
// typedef struct BBManager {
//     BounceBuffer *buffers[MAX_BUFFERS];
//     int num_buffers;
//     pthread_mutex_t lock;
// } BBManager;

static BounceBuffer bounce;
// static BBManager bbm = {
//     .num_buffers = 0,
...
* likely to succeed.
*/
// #define pr_fmt(fmt) "%s:%d(%s)"fmt"\n", __FILE__, __LINE__, __func__
// static


// #include "hw/femu/femu.h"
// static void *femu_get_ptr(FemuCtrl *femu, hwaddr addr, hwaddr len)
// {
//     void *ptr = NULL;
//     off_t offset = 0;
//     if (femu->base_gpa <= addr && addr+len <= femu->base_gpa+femu->mbe->size) {
//         ptr = femu->mbe->logical_space;
//         offset = addr - femu->base_gpa;
//     }
//     return ptr+offset;
// }
                        ...
                        MemTxAttrs attrs)
{
    ...
    fv = address_space_to_flatview(as);
    mr = flatview_translate(fv, addr, &xlat, &l, is_write, attrs);
    // DEBUG(addr, "%s mr: %s addr:0x%lx, len:0x%lx l:0x%lx\n", __func__, memory_region_name(mr), addr, len, l);
    // if (mr->dual_mode) {
    // // // if (!memory_access_is_direct(mr, is_write)) {
    //     BounceBuffer *bb = NULL;
    //     bool found = false;
    //     pthread_mutex_lock(&bbm.lock);
        
    //     for (int idx = 0; idx < bbm.num_buffers; idx++) {
    //         bb = bbm.buffers[idx];
    //         if (bb->in_use) {
    //             continue;
    //         }
    //         else {
    //             // printf("addr: 0x%lx, found. idx: %d\n",addr, idx);
    //             found = true;
    //             break;
    //         }
    //     }
    ...
    //     if (!found) {
    //         // printf("addr: 0x%lx, create new. idx: %d\n",addr, bbm.num_buffers);
    //         bb = bbm.buffers[bbm.num_buffers] = malloc(sizeof(BounceBuffer));
    //         if (bb == NULL) {
    //             *plen = 0;
    //             pthread_mutex_unlock(&bbm.lock);
    //             return NULL;
    //         }

    //         bbm.num_buffers++;
    //         if (bbm.num_buffers > MAX_BUFFERS) {
    //             *plen = 0;
    //             pthread_mutex_unlock(&bbm.lock);
    //             return NULL;
    //         }
    ...
    //     bb->buffer = qemu_memalign(TARGET_PAGE_SIZE, l);
    //     bb->raw = femu_get_ptr((FemuCtrl *)mr->owner, addr, l);
    //     bb->addr = addr;
    //     bb->len = l;
    ...
    //     if (!is_write) {
    //         memcpy(bb->buffer, bb->raw, bb->len);
    //     }

    //     *plen = l;
    //     // printf("bb(%p): %p\n",bb, bb->buffer);
    //     DEBUG(addr, "%s addr: 0x%lx, buffer:0x%lx, raw:0x%lx\n",__func__, addr, (uint64_t)bb->buffer, (uint64_t)bb->raw);
    //     // printf("% s addr:0x%lx, len:0x%lx, write:%d\n",__func__, addr, *plen, is_write);
    //     for (int i = 0; i<256; i+=sizeof(uint64_t)) {
    //         if (i%32 == 0) {
    //             printf("\n");
    //         }
    //         printf("%016lx.", *(uint64_t *)(bb->raw+i));
...
#include "hw/femu/femu.h"
    if (mr->dual_mode) {
        ...
        while (l > 0) {
            ...
            femu->cxl_mem_ops.read((void *)femu, offset, &dummy, sizeof(dummy), MEMTXATTRS_DUAL_MODE_DMA_MAP);
            offset += size;
            l -= size;
        }
        // // femu->cxl_mem_ops.read((void *)femu, offset, &dummy, sizeof(dummy), MEMTXATTRS_UNSPECIFIED);
        *plen = len;
        // printf("%s, addr: 0x%lx, ptr: %p, value: 0x%lx\n", __func__, addr, ptr, *(uint64_t *)ptr);
        return ptr;
...
* Will also mark the memory as dirty if is _write is true.  access_len gives
    ...
    if (buffer != bounce.buffer) {
        ...
        if (mr->dual_mode) {
            ...
            while (l > 0) {
                ...
                femu->cxl_mem_ops.write((void *)femu, offset, dummy, sizeof(dummy), MEMTXATTRS_DUAL_MODE_DMA_MAP);
                offset += size;
                l -= size;
            }
            memory_region_unref(mr);
            // printf("%s, addr: 0x%lx, ptr: %p, value: 0x%lx, dummy: 0x%lx\n", __func__, addr1, buffer, *(uint64_t *)buffer, dummy);
            return;
        }
        ...
        // if (!mr) {
        //     for (int idx = 0; idx < bbm.num_buffers; idx++) {
        //         if (bbm.buffers[idx]->buffer == buffer) {
        //             bptr = bbm.buffers[idx];
        //             break;
        //         }
        //     }

        //     if (bptr == &bounce) {
        //         // printf("raw base: 0x%lx\n", (uint64_t)femu_get_ptr((FemuCtrl *)mr->owner, ((FemuCtrl *)mr->owner)->base_gpa, 1024));
        //         printf("bounce: addr:0x%lx, buffer:0x%lx, raw:0x%lx, len:0x%lx, in_use:%d\n", bounce.addr, (uint64_t)bounce.buffer, (uint64_t)bounce.raw, bounce.len, bounce.in_use);
        //         for (int idx = 0; idx < bbm.num_buffers; idx++) {
        //             if (bbm.buffers[idx]->buffer == buffer) {
        //                 printf("bbm[%d]: addr:0x%lx, buffer:0x%lx, raw:0x%lx, len:0x%lx, in_use:%d\n",idx, bbm.buffers[idx]->addr, (uint64_t)bbm.buffers[idx]->buffer, (uint64_t)bbm.buffers[idx]->raw, bbm.buffers[idx]->len, bbm.buffers[idx]->in_use);
        //             }
        //         }   
        //     }

        //     assert(bptr != &bounce);
        //     DEBUG(bptr->addr, "%s addr: 0x%lx, buffer:0x%lx, raw: 0x%lx, \n",__func__, bptr->addr, (uint64_t)bptr->buffer, (uint64_t)bptr->raw);
        //     mr = bptr->mr;
        // }

        // if (is_write) {
        //     memcpy(bptr->raw, bptr->buffer, bptr->len);
        // }

        // if (mr->dual_mode) {
        //     printf("%s buffer:0x%lx, raw:0x%lx len:0x%lx, write:%d\n",__func__, (uint64_t)bptr->buffer, (uint64_t)bptr->raw, len, is_write);
        //     for (int i = 0; i<256; i+=sizeof(uint64_t)) {
        //         if (i%32 == 0) {
        //             printf("\n");
        //         }
        ...
        //     attr = MEMTXATTRS_DUAL_MODE_DMA;
        // }
        // mr = bptr->mr;
        // offset = bptr->addr - mr->addr;
        // assert(offset < memory_region_size(mr));
        // if (offset > memory_region_size(mr)) {
        //     printf("mr.size: 0x%lx, offset: 0x%lx\n", memory_region_size(mr), offset);
        //     printf("\taddr: 0x%lx, mr.name: %s, mr.addr: 0x%lx, base ptr: %p\n",bptr->addr, memory_region_name(mr), mr->addr, mr->ram_block->host);
        // }

        // ptr = memory_region_get_ram_ptr(mr) + offset;
        
        
        // qemu_vfree(bptr->buffer);
        // bptr->buffer = NULL;
        // bptr->raw = NULL;
        // memory_region_unref(bptr->mr);
        // /* Clear in_use before reading map_client_list.  */
        // qatomic_set_mb(&bptr->in_use, false);
