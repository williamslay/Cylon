...
#define DEBUG(x, fmt, ...) do { \
        if (x >= 0x2290000000 && x < 0x3a8fffffff+1) { \
            printf(fmt, ##__VA_ARGS__); fflush(stdout);\
        }\
    } while(0);
/*
// #define BREAK_POINT(x) do { \
//         if (x >= 0x2290000000 && x < 0x3a8fffffff+1) { \
//             raise(SIGUSR2);\
//         }\
//     } while(0);
...
static void __bp(void) {
    /* */
    return;
}

static void BREAK_POINT(dma_addr_t x)
{
    if (x >= 0x2290000000 && x < 0x3a8fffffff+1) {
        __bp();
    }
...
static void dma_blk_cb(void *opaque, int ret)
{
    ...
    if (dbs->sg_cur_index == dbs->sg->nsg || ret < 0) {
        // DEBUG(cur_addr, "%s addr:0x%lx ret;%d\n", __func__, cur_addr, ret);
        // printf("%s %d\n",__func__, ret);
        // BREAK_POINT(cur_addr);
        // ret = 0;
        dma_complete(dbs, ret);
        if (ret < 0) {
            DEBUG(0x2290000000, "%s ret:%d\n", __func__, ret);
            BREAK_POINT(0x2290000000);
        }
    ...
    while (dbs->sg_cur_index < dbs->sg->nsg) {
         ...
         */
        // DEBUG(cur_addr, "%s addr:0x%lx mem:0x%lx len:0x%lx\n", __func__, cur_addr, (uint64_t)mem, cur_len);
        // BREAK_POINT(cur_addr);
