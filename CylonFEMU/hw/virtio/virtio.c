                               ...
                               hwaddr pa, size_t sz)
{
    ...
    while (sz) {
        ...
        //     ;
        // }
        // printf("%s, addr:0x%lx, pa:0x%lx, num_sg: %d, size:0x%lx ", __func__, (uint64_t)addr, (uint64_t)pa, num_sg, sz);
        // printf("[%s] [%s] [%s] dma_base:0x%lx pa:0x%lx\n", vdev->name, vdev->bus_name, vdev->dma_as->name, (uint64_t)vdev->dma_as->root->addr, (uint64_t)pa);
        ...
        if (!iov[num_sg].iov_base) {
            ...
            printf("%s, addr:0x%lx, pa:0x%lx, num_sg: %d, size:0x%lx\n", __func__, (uint64_t)addr, (uint64_t)pa, num_sg, sz);
            printf("[%s] [%s] [%s] dma_base:0x%lx pa:0x%lx\n", vdev->name, vdev->bus_name, vdev->dma_as->name, (uint64_t)vdev->dma_as->root->addr, (uint64_t)pa);
            sleep(10000);
            abort();
            goto out;
...
void virtio_init(VirtIODevice *vdev, uint16_t device_id, size_t config_size)
{
    ...
    printf("%s: [%s] [%s] \n",__func__, vdev->name, vdev->bus_name);
