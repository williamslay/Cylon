                      ...
                      BdrvRequestFlags flags)
{
    ...
    if (ret < 0) {
        printf("blk_check_byte_request error\n");
        return ret;
    }
...
static void coroutine_fn blk_aio_read_entry(void *opaque)
{
    ...
    if (rwco->ret < 0) {
        printf("%s\n\tbackend name:%s, offset:0x%lx, iobuf:0x%lx, ret:%d, flag: %d\n",__func__, rwco->blk->name, rwco->offset, (uint64_t)rwco->iobuf, rwco->ret, rwco->flags);
        printf("\tacb bytes: 0x%lx, returned(%d), driver:%s\n", acb->bytes, acb->has_returned, acb->common.bs->drv->format_name);
        printf("\tqiov niov: %d\n", qiov->niov);
        for (int i = 0; i < qiov->niov; i++) {
            printf("\tqiov iov[%d]: base: 0x%lx, len:0x%lx, \n", i, (uint64_t)qiov->iov[i].iov_base, qiov->iov[i].iov_len);
        }
    }
