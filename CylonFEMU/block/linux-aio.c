...
static void qemu_laio_process_completions(LinuxAioState *s)
{
    ...
    while ((s->event_max = io_getevents_advance_and_peek(s->ctx, &events,
        ...
        for (s->event_idx = 0; s->event_idx < s->event_max; ) {
            ...
            if (laiocb->ret < 0) {
                printf("io event ret: %ld\n", laiocb->ret);
                printf("\t nbytes:0x%lx, is_read:%d, niov:%d\n", laiocb->nbytes, laiocb->is_read, laiocb->qiov->niov);
                for (int i =0; i<laiocb->qiov->niov; i++) {
                    printf("\t\tiov[%d]: base:0x%lx, len:0x%lx\n", i, (uint64_t)laiocb->qiov->iov[i].iov_base, laiocb->qiov->iov[i].iov_len);
                }
