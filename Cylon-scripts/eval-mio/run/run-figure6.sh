...
TARGET="${TARGET:-Cylon-FIFO-validation}"
MEM_NODE=1
MIN_NS="${MIN_NS:-0.0}"
MAX_NS="${MAX_NS:-1.0}"
...
if [[ -z "${PLOT_GP:-}" ]]; then
  if [[ -f "${TOPDIR}/gp/plot.gp" ]]; then
    PLOT_GP="${TOPDIR}/gp/plot.gp"
  elif [[ -f "${TOPDIR}/run/plot.gp" ]]; then
    PLOT_GP="${TOPDIR}/run/plot.gp"
  elif [[ -f "${TOPDIR}/plot.gp" ]]; then
    PLOT_GP="${TOPDIR}/plot.gp"
  else
