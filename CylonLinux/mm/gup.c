...
static int check_vma_flags(struct vm_area_struct *vma, unsigned long gup_flags)
{
	...
	if (vm_flags & (VM_IO | VM_PFNMAP)) {
		// printk("VM_IO | VM_PFNMAP start:0x%lx\n",vma->vm_start);
		// dump_stack();
		return -EFAULT;
	}
	...
	do {
		...
		if (!vma || start >= vma->vm_end) {
			...
			if (vma && (start < vma->vm_start)) {
				...
				// printk("%s:%d find_vma failed. vm_addr[0x%lx - 0x%lx] flags:0x%lx\n", __FILE__, __LINE__, vma->vm_start, vma->vm_end, vma->vm_flags);
				// dump_stack();
			...
			if (!vma && in_gate_area(mm, start)) {
				ret = get_gate_page(mm, start & PAGE_MASK,
						...
						pages ? &pages[i] : NULL);
				if (ret) {
					// printk("%s:%d get_gate_page failed(%d). vm_addr[0x%lx - 0x%lx] flags:0x%lx\n", __FILE__, __LINE__,ret, vma->vm_start, vma->vm_end, vma->vm_flags);
					// dump_stack();
					goto out;
				}
			...
			if (ret) {
				// printk("%s:%d check_vma_flags failed(%d). vm_addr[0x%lx - 0x%lx] flags:0x%lx // 0x%lx\n", __FILE__, __LINE__, ret, vma->vm_start, vma->vm_end, vma->vm_flags, gup_flags);
				// dump_stack();
				goto out;
			}
...
retry:
		...
		if (!page || PTR_ERR(page) == -EMLINK) {
			...
			case -EAGAIN:
				...
				fallthrough;
			case -EFAULT:
				// printk("%s:%d faultin_page failed. vm_addr[0x%lx - 0x%lx] flags:0x%lx\n", __FILE__, __LINE__, vma->vm_start, vma->vm_end, vma->vm_flags);
				// dump_stack();
			case -ENOMEM:
			case -EHWPOISON:
				goto out;
		...
		unsigned int foll_flags)
{
	...
	for (i = 0; i < nr_pages; i++) {
		...
		if ((vma->vm_flags & (VM_IO | VM_PFNMAP)) ||
			...
			// printk("%s:%d vma wrong flag. vm_addr[0x%lx - 0x%lx] flags:0x%lx // 0x%lx\n", __FILE__, __LINE__, vma->vm_start, vma->vm_end, vma->vm_flags, vm_flags);
			// dump_stack();
			break;
		}
					...
					struct page **pages)
{
	...
	if (end > TASK_SIZE_MAX) {
		// printk("%s:%d uaddr: 0x%lx, len: 0x%lx\n", __FILE__, __LINE__,(unsigned long)start, len);
		return -EFAULT;
	}
		
	if (unlikely(!access_ok((void __user *)start, len))) {
		// printk("%s:%d access not ok. uaddr: 0x%lx, len: 0x%lx\n", __FILE__, __LINE__,(unsigned long)start, len);
		return -EFAULT;
	}
	...
	if (ret < 0) {
		 ...
		 */
		// printk("%s:%d pinned not ok. uaddr: 0x%lx, pages: 0x%lx\n", __FILE__, __LINE__,(unsigned long)start, pages);
