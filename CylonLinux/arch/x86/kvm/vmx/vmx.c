...
// {
// 	return (addr >= 0x491200000 && addr < 0x491200080) || (addr >= 0x498000000 && addr < 0x498000080);
// }
...
static int handle_ept_violation(struct kvm_vcpu *vcpu)
{
	...
	// if (is_cxl_memregion(gpa))
	// 	printk("handle_ept_violation. addr: %llx", gpa);
	// if (gpa == 0x3690afa000)
	// printk("%s: gpa: 0x%llx\n", __func__, gpa);
...
static int __vmx_handle_exit(struct kvm_vcpu *vcpu, fastpath_t exit_fastpath)
{
	...
	// if (is_cxl_memregion(gpa)) {
	// 	printk("__vmx_handle_exit. addr: %llx, reason:%d", gpa, exit_reason.basic);
	// }
