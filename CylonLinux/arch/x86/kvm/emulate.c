...
#define __DEBUG_TARGET 0x3690b72000
// 0x3600064000
...
int x86_decode_insn(struct x86_emulate_ctxt *ctxt, void *insn, int insn_len, int emulation_type)
{
	...
	if (insn_len > 0)
		...
		// printk("gpa:0x%llx(%d) rc:%d\n", ctxt->gpa_val, ctxt->gpa_available, rc);
		if (rc != X86EMUL_CONTINUE) {
			// printk("[%d] gpa:0x%llx(%d) rc:%d\n",cnt, ctxt->gpa_val, ctxt->gpa_available, rc);
			// if (kvm_vcpu_gfn_to_memslot(ctxt->vcpu, ctxt->gpa_val >> 12)->flags & KVM_MEMSLOT_DUAL_MODE) {
			// 	u64 tmp;		
			// 	// printk("\t fetch data and retry\n");
			// 	// read_emulated(ctxt, ctxt->gpa_val, &tmp, sizeof(tmp));
...
#endif
	default:
		printk("[%d] Invalid mode: gpa:0x%llx(%d) rc:%d\n",cnt, ctxt->gpa_val, ctxt->gpa_available, rc);
	...
	for (;;) {
		...
		case 0x66:	/* operand-size override */
			// __DEBUG(ctxt->gpa_val, "\t[%d] 0x66\n", cnt);
			op_prefix = true;
			/* switch between 2/4 bytes */
			ctxt->op_bytes = def_op_bytes ^ 6;
			break;
		case 0x67:	/* address-size override */
			// __DEBUG(ctxt->gpa_val, "\t[%d] 0x67\n", cnt);
		...
		case 0x26:	/* ES override */
			// __DEBUG(ctxt->gpa_val, "\t[%d] 0x26\n", cnt);
			has_seg_override = true;
			ctxt->seg_override = VCPU_SREG_ES;
			break;
		case 0x2e:	/* CS override */
			// __DEBUG(ctxt->gpa_val, "\t[%d] 0x2e\n", cnt);
			has_seg_override = true;
			ctxt->seg_override = VCPU_SREG_CS;
			break;
		case 0x36:	/* SS override */
			// __DEBUG(ctxt->gpa_val, "\t[%d] 0x36\n", cnt);
			has_seg_override = true;
			ctxt->seg_override = VCPU_SREG_SS;
			break;
		case 0x3e:	/* DS override */
			// __DEBUG(ctxt->gpa_val, "\t[%d] 0x3e\n", cnt);
			has_seg_override = true;
			ctxt->seg_override = VCPU_SREG_DS;
			break;
		case 0x64:	/* FS override */
			// __DEBUG(ctxt->gpa_val, "\t[%d] 0x64\n", cnt);
			has_seg_override = true;
			ctxt->seg_override = VCPU_SREG_FS;
			break;
		case 0x65:	/* GS override */
			// __DEBUG(ctxt->gpa_val, "\t[%d] 0x65\n", cnt);
			has_seg_override = true;
			ctxt->seg_override = VCPU_SREG_GS;
			break;
		case 0x40 ... 0x4f: /* REX */
			// __DEBUG(ctxt->gpa_val, "\t[%d] 0x40\n", cnt);
			if (mode != X86EMUL_MODE_PROT64)
				goto done_prefixes;
		...
		// 	break;
		case 0xf0:	/* LOCK */
			// __DEBUG(ctxt->gpa_val, "\t[%d] 0xf0\n", cnt);
			ctxt->lock_prefix = 1;
			break;
		case 0xf2:	/* REPNE/REPNZ */
		case 0xf3:	/* REP/REPE/REPZ */
			// __DEBUG(ctxt->gpa_val, "\t[%d] 0xf3 or 0xf2\n", cnt);
			ctxt->rep_prefix = ctxt->b;
			break;
		
		default:
			// __DEBUG(ctxt->gpa_val, "\t[%d] default\n", cnt);
			goto done_prefixes;
		}
...
done_prefixes:
	...
	// __DEBUG(ctxt->gpa_val, "\t[%d] opcode idx: 0x%x\n", cnt, ctxt->b);
	...
	while (ctxt->d & GroupMask) {
		...
		case ModeDual:
			...
			break;
		default:
			printk("[%d] Invalid group mask: gpa:0x%llx(%d) rc:%d\n",cnt,  ctxt->gpa_val, ctxt->gpa_available, rc);
			return EMULATION_FAILED;
		}
	...
	if (rc != X86EMUL_CONTINUE) {
		printk("[%d] Decode failed: gpa:0x%llx(%d) rc:%d\n",cnt, ctxt->gpa_val, ctxt->gpa_available, rc);
		goto done;
	}
	...
	if (rc != X86EMUL_CONTINUE) {
		printk("[%d] Decode src failed: gpa:0x%llx(%d) rc:%d\n",cnt, ctxt->gpa_val, ctxt->gpa_available, rc);
		goto done;
	}
	...
	if (rc != X86EMUL_CONTINUE) {
		printk("[%d] Decode src2 failed: gpa:0x%llx(%d) rc:%d\n",cnt, ctxt->gpa_val, ctxt->gpa_available, rc);
		goto done;
	}
