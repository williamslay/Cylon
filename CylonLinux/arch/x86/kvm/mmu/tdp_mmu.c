...
int kvm_tdp_mmu_map(struct kvm_vcpu *vcpu, struct kvm_page_fault *fault)
{
	...
	tdp_mmu_for_each_pte(iter, mmu, fault->gfn, fault->gfn + 1) {
		...
		/* Make CXL emulated memory region linear-mapped*/
...
map_target_level:
	...
	// if (fault->slot != NULL
	// 			&& fault->slot->flags & KVM_MEMSLOT_DUAL_MODE) {
	// 	printk("gfn: 0x%llx, iter.gfn: 0x%llx, iter level: %d, sp->spt: 0x%llx, ret: %d\n", fault->gfn, iter.gfn, iter.level, (u64)sp->spt, ret);
	// }
retry:
	rcu_read_unlock();
	return ret;
...
int kvm_arch_vm_ioctl_set_spte_flag(struct kvm *kvm, struct kvm_set_spte_flag *data)
{
	 ...
	 * into this helper allow blocking; it'd be dead, wasteful code.
	 */
	// printk("vm set_spte_flag");
	for (as_id = 0; (as_id <= KVM_ADDRESS_SPACE_NUM && !r); as_id++)
		for_each_tdp_mmu_root(kvm, root, as_id) {
			tdp_root_for_each_leaf_pte(iter, root, gfn, gfn+1) {
				...
				spte &= ~(u64)0xFFF;
				spte |= flag;
				// printk("\tCLEAR mmio bit. as_id:%llx, gpa: %llx, spte:%llx -> %llx, flag: %llx", (u64)iter.as_id, addr, *sptep, spte, flag);	
