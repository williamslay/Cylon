			...
			kvm_pfn_t pfn, struct kvm_page_fault *fault)
{
	...
	if (unlikely(is_noslot_pfn(pfn))) {
		...
		// if (is_cxl_memregion(gfn << PAGE_SHIFT))
		// 	printk("mark mmio spte. sptep:%llx, gfn:%llx, access: %x",(u64)sptep, gfn, pte_access);
...
static bool mmio_info_in_cache(struct kvm_vcpu *vcpu, u64 addr, bool direct)
{
	...
	if (direct){
		r =	vcpu_match_mmio_gpa(vcpu, addr);
		// return vcpu_match_mmio_gpa(vcpu, addr);
		// if (is_cxl_memregion(addr)){
		// 	printk("mmio_info_in_cache. addr: %llx, RES: %d", addr, r);
		// }
		return r;
	}
...
static bool get_mmio_spte(struct kvm_vcpu *vcpu, u64 addr, u64 *sptep)
{
	...
	*sptep = sptes[leaf];
	...
	// if (reserved) {
	// 	printk("%s: reserved bits set on MMU-present spte, addr 0x%llx, spte 0x%llx, hierarchy:\n",
	// 	       __func__, addr, *sptep);
	// 	for (level = root; level >= leaf; level--)
	// 		printk("------ spte = 0x%llx level = %d, rsvd bits = 0x%llx",
	// 		       sptes[level], level,
...
// #include <asm/io.h>
// // static char *tmp_pg_sp;
// char tmp_pg_sp[4096*2];
// void *tmp_pg___ = NULL;
// bool start___ = true;
// struct task_struct *kthread;
// static void print_____(void)
// {
// 	int i;
// 	for(i=0;i<4;i++){
// 		printk("\ttmp[%d]: %llx",i, ((u64 *)tmp_pg___)[i]);
// 	}
// }

// static int thread_print___(void* arg)
// {
// 	while(start___){
// 		msleep(1000);
// 	}
...
// static bool roundup___(void)
// {
// 	// int fd = open("/dev/dax0.0", O_RDWR | O_CREAT);
// 	// if (fd < 1)
// 	// 	return false;
	
// 	// tmp_pg_sp = mmap(0, 4096*2, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
// 	// if (tmp_pg_sp == MAP_FAILED) 
// 	// 	return false;

// 	tmp_pg___ = (void *)((((u64)tmp_pg_sp >> 12) +1) << 12);
// 	printk("va: %llx, pa: %llx, rva:%llx, rpg: %llx", (u64)tmp_pg_sp, virt_to_phys(tmp_pg_sp), (u64)tmp_pg___, virt_to_phys(tmp_pg___));
	
// 	kthread = kthread_run(thread_print___, NULL, "printttt");
// 	if (kthread == NULL)
// 		printk("kthread failed");
...
static int handle_mmio_page_fault(struct kvm_vcpu *vcpu, u64 addr, bool direct)
{
	...
	if (slot && (slot->flags & KVM_MEMSLOT_DUAL_MODE)) {
		vcpu->arch.emulate_ctxt->gpa_val = addr;
		vcpu->arch.emulate_ctxt->gpa_available = true;
		// return RET_PF_EMULATE;
	}
				...
				u64 fault_address, char *insn, int insn_len)
{
	...
	// if (is_cxl_memregion(fault_address))
	// 	printk("kvm_handle_page_fault. addr: %llx", fault_address);
		       ...
		       void *insn, int insn_len)
{
	...
	// 	printk("kvm_mmu_page_fault. addr: %llx, direct:%d", cr2_or_gpa, direct);
...
{
	// static_call(kvm_x86_flush_tlb_gva)(vcpu,);
...
int kvm_arch_vm_ioctl_get_linear_spt(struct kvm *kvm, struct kvm_memslot_get_linear_spt *data)
{
	...
	// 	struct vm_area_struct *vma = find_vma(mm, (u64)data->backend_ptr);
	// 	struct page* page;
	// 	unsigned long uaddr, pfn;
	// 	int r;
	// 	if (vma != NULL) {
			
	// 		unmap_mapping_range(vma->vm_file->f_mapping, vma->vm_start, vma->vm_end - vma->vm_start, 1);
	// 		printk("Clear vma flags. buffer addr: 0x%llx\n", (u64)data->backend_ptr);
	...
	// 			pfn = vma->vm_pgoff + ((uaddr - vma->vm_start) >> PAGE_SHIFT);
	// 			if (!pfn_valid(pfn)) {
	// 				pr_err("Invalid PFN at 0x%lx\n", uaddr);
	// 				return -EINVAL;
	// 			}

	// 			page = pfn_to_page(pfn);
	// 			if (page == NULL) {
	// 				pr_err("pfn_to_page failed at 0x%lx\n", uaddr);
	// 				return -EFAULT;
	// 			}

				
	// 			r = vm_insert_page(vma, uaddr, page);
	// 			if (r) {
	// 				pr_err("vm_insert_page failed (%d) at 0x%lx (pfn:0x%lx)\n", r, uaddr, pfn);
					
	// 				r = vmf_insert_pfn(vma, uaddr, pfn);
	// 				if (r) {
	// 					pr_err("vm_insert_pfn failed (%d) at 0x%lx (pfn:0x%lx)\n", r, uaddr, pfn);
	// 					// return -EFAULT;
	// 					continue;
	// 				}
	// 				return -EFAULT;
