...
u64 make_dual_mode_spte(struct kvm_vcpu *vcpu, u64 gfn, unsigned int access)
{
	u64 gen = kvm_vcpu_memslots(vcpu)->generation & MMIO_SPTE_GEN_MASK;
	u64 spte = generation_mmio_spte_mask(gen);
	u64 gpa = gfn << PAGE_SHIFT;
...
out:
	...
	WARN_ONCE( !(slot->flags & KVM_MEMSLOT_DUAL_MODE) && is_rsvd_spte(&vcpu->arch.mmu->shadow_zero_check, spte, level),
